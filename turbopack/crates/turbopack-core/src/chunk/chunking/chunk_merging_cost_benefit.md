...
\begin{flalign*}
& \begin{aligned}
\text{paired}_x &= c_{xx} + c_{xy} + c_{xz} \\
\text{paired}_y &= c_{xy} + c_{yy} + c_{yz} \\
\text{paired}_z &= c_{xz} + c_{yz} + c_{zz}
\end{aligned} &
\end{flalign*}
$$
...
\begin{flalign*}
& \begin{aligned}
\text{non\_paired}_{12} &= |S_1| \cdot |S_2| - c_{12} \\
\text{non\_paired}_1 &= (\text{groups} - 1) \cdot |S_1| - \text{paired}_1
\end{aligned} &
\end{flalign*}
$$

When set 1 and set 2 are the same set, $|S_2|$ becomes $|S_2| - 1$, since a group cannot navigate to itself.

Therefore:

$$
\begin{flalign*}
& \text{trans}(1 \to 2) = \text{cnp} \cdot (c_{12} / \text{paired}_1) + (1 - \text{cnp}) \cdot (\text{non\_paired}_{12} / \text{non\_paired}_1) &
\end{flalign*}
$$

To prevent dividing by zero, if $\text{paired}_1 = 0$ then
$\text{trans}(1 \to 2) = |S_2| / (\text{groups} - 1)$, and if $\text{non\_paired}_1 = 0$
then $\text{trans}(1 \to 2) = c_{12} / \text{paired}_1$.
