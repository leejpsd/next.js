...
use swc_core::{
    ...
    ecma::{
        ast::{
            ...
            SourceMapperExt, Stmt, VarDecl, VarDeclKind, VarDeclarator,
        },
...
impl MergeableModule for EcmascriptModuleAsset {
    ...
    async fn is_mergeable(self: ResolvedVc<Self>) -> Result<Vc<bool>> {
        ...
        // hoisting is enabled.
        if self.options().await?.cjs_scope_hoisting
            && matches!(
                &*self.get_exports().await?,
            ...
            GLOBALS.set(globals, || {
                ...
                impl VisitMut for CjsExportRewriter {
                    fn visit_mut_expr(&mut self, expr: &mut Expr) {
                        ...
                        // A merged module has no `exports` object to mark, and the group
                        // registers the flag itself. Any other `Object.defineProperty` export
                        // keeps the module out of a group, so only `__esModule` reaches here.
                        ...
                        // `module.exports = { a, b: c }` assigns every export at once, so
                        // rewrite it to one assignment per local: `(_a = a, _b = c)`.
                ...
                impl CjsExportRewriter {
                    /// Rewrites `module.exports = { a, b: c }` into `(_a = a, _b = c)`, so each
                    /// export ends up in the local the merge reads it through. A property with
                    /// no local (dropped as unused) keeps only its value, for the side effects.
                    /// Returns `None` for anything that isn't a whole-exports object literal of
                    ...
                    fn rewrite_whole_exports_object(
                        ...
                        // Every property has to name one export and carry an eager value; a
                        // spread, getter, setter or method does not.
                        if !obj.props.iter().all(|prop| {
                            matches!(prop, PropOrSpread::Prop(prop)
                                if matches!(&**prop, Prop::Shorthand(_) | Prop::KeyValue(_)))
                        }) {
                            return None;
                        }
                                ...
                                let (name, value) = match *prop {
                                    ...
                                    Prop::KeyValue(KeyValueProp { key, value, .. }) => {
                                        let name = match &key {
                                            ...
                                            // A computed or numeric key names no export.
                                            _ => return value,
                                        };
                                        (name, value)
                                    }
